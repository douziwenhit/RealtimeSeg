...
def main(pretrain=True):
    ...
    ohem_criterion = nn.CrossEntropyLoss(reduction='mean',
                                    ignore_index=255)
    ...
    if type(pretrain) == str:
        partial = torch.load("./pretrain-256x512_F12.L16_batch1/weights.pt", map_location='cuda:0')
    ...
    train_loader_model = get_train_loader(config, ADE, portion=config.train_portion, index_select=index_select)
    train_loader_arch = get_train_loader(config, ADE, portion=config.train_portion-1, index_select=index_select)

    evaluator = SegEvaluator(ADE(data_setting, 'val', None), config.num_classes, config.image_mean,
            ...
            for idx in range(len(config.flops_weight)):
                if config.flops_weight[idx] > 0:
                    if (int(FPSs[idx][0] >= config.FPS_max[idx]) + int(FPSs[idx][1] >= config.FPS_max[idx])) >= 1:
                        ...
                        architect.flops_weight[idx] *= 2
                    logger.add_scalar("arch/latency_weight_%s"%arch_names[idx], architect.flops_weight[idx], epoch+1)
