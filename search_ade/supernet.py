    ...
    def _backward_step(self, input_valid, target_valid):
        ...
        for idx in range(len(self.optimizers)):
            ...
            if self.latency_weight == 0:
                ...
                if len(self.model._width_mult_list) == 1:
                    r0 = 1./500; r1 = 499./500
                    latency = latency + r0 * self.model.forward_latency((3, 1024, 2048), fai=True, beta=False, ratio=False)
                    latency = latency + r1 * self.model.forward_latency((3, 1024, 2048), fai=False, beta=True, ratio=False)
                else:
                    r0 = 1./500; r1 = 497./500; r2 = 2./500
                    latency = latency + r0 * self.model.forward_latency((3, 1024, 2048), fai=True, beta=False, ratio=False)
                    latency = latency + r1 * self.model.forward_latency((3, 1024, 2048), fai=False, beta=True, ratio=False)
                    latency = latency + r2 * self.model.forward_latency((3, 1024, 2048), fai=False, beta=False, ratio=True)
                self.latency_supernet = latency
                loss_latency = loss_latency + latency * self.latency_weight
        for idx_flops in range(len(self.optimizers)):
            ...
            if self.flops_weight == 0:
                ...
                if len(self.model._width_mult_list) == 1:
                    # r0 = 1/2; r1 = 1/2
                    r0 = self._args.fai_weight
                    r1 = self._args.beta_weight
                    flops = flops + r0 * self.model.forward_flops((3, 1024, 2048), fai=True, beta=False, ratio=False)
                    flops = flops + r1 * self.model.forward_flops((3, 1024, 2048), fai=False, beta=True, ratio=False)
                else:
                    ...
                    flops = flops + r0 * self.model.forward_flops((3, 1024, 2048), fai=True, beta=False, ratio=False)
                    flops = flops + r1 * self.model.forward_flops((3, 1024, 2048), fai=False, beta=True, ratio=False)
                    flops = flops + r2 * self.model.forward_flops((3, 1024, 2048), fai=False, beta=False, ratio=True)

                self.flops_supernet = flops
                if flops < 1.3e11:
                   self._args.Flops = 0
                loss_flops = loss_flops + flops * self.flops_weight
