...
import torchvision.transforms as T
from tools.datasets import Cityscapes
filename = '/home/wangshuo/douzi/AutoSeg_edge/pytorch2trt/test.jpg'
max_batch_size = 1
onnx_model_path = '/home/wangshuo/douzi/AutoSeg_edge/pytorch2trt/seg21.onnx'

TRT_LOGGER = trt.Logger()  # This logger is required to build an engine
#//dataset//////////////////////
"""Data Dir"""
dataset_path = "/home/wangshuo/douzi/pytorch-multigpu/cityscapes"
...
file_path_train = "/home/wangshuo/douzi/pytorch-multigpu/cityscapes/leftImg8bit/test/berlin"  #文件路径
path_list_train = os.listdir(file_path_train) #遍历整个文件夹下的文件name并返回一个列表
path_name_train = []#定义一个空列表

for i in path_list_train:
    path_name_train.append(i.split(".")[0]) #若带有后缀名，利用循环遍历path_list列表，split去掉后缀名
path_name_train.sort() #排序
for file_name in path_name_train:
     # Do inference
     shape_of_output = (1,19,1024,2048)
     # Load data to the buffer
     img_scale = load_data('/home/wangshuo/douzi/pytorch-multigpu/cityscapes/leftImg8bit/test/berlin/'+ file_name +".png")
     ...
     image = cv2.imread('/home/wangshuo/douzi/pytorch-multigpu/cityscapes/leftImg8bit/test/berlin/'+ file_name +".png")
     comp_img = show_prediction(colors, -1, image, pred)
     cv2.imwrite(os.path.join(os.path.realpath('.'), save_path, "test", file_name+".viz.png"), comp_img[:,:,::-1])
     for x in range(pred.shape[0]):
         for y in range(pred.shape[1]):
            pred[x, y] = cityscapes_trainID2id[pred[x, y]]
     cv2.imwrite(os.path.join(os.path.realpath('.'), save_path, "test", file_name+".png"), pred)
