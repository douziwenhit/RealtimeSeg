...
class MixedOp(nn.Module):
    ...
    def forward_flops(self, size, alpha, ratio):
        # int: force #channel; tensor: arch_ratio; float(<=1): force width
        result = 0
...
class Cell(nn.Module):
    ...
    def forward_flops(self, size, alphas, ratios):
        # ratios: (in, out, down)
        out = self._op.forward_flops(size, alphas, (ratios[0], ratios[1]))
        assert (self._down and (ratios[2] is not None)) or ((not self._down) and (ratios[2] is None))
        down = self.downsample.forward_latency(size, alphas, (ratios[0], ratios[2])) if self._down else None
        return out, down
...
class Network_Multi_Path(nn.Module):
    ...
    def forward(self, input):
        ...
        alphas0 = F.softmax(getattr(self, self._arch_names[self.arch_idx]["alphas"][0]), dim=-1).cuda()
        alphas1 = F.softmax(getattr(self, self._arch_names[self.arch_idx]["alphas"][1]), dim=-1).cuda()
        alphas2 = F.softmax(getattr(self, self._arch_names[self.arch_idx]["alphas"][2]), dim=-1).cuda()
        alphas = [alphas0, alphas1, alphas2]
        betas1 = F.softmax(getattr(self, self._arch_names[self.arch_idx]["betas"][0]), dim=-1).cuda()
        betas2 = F.softmax(getattr(self, self._arch_names[self.arch_idx]["betas"][1]), dim=-1).cuda()
    ...
    def forward_flops(self, size, alpha=True, beta=True, ratio=True):
        ...
        for i, cells in enumerate(self.cells):
            ...
            for j, cell in enumerate(cells):
                ...
                # out,down -- 0: from down; 1: from keep
                if j == 0:
                    out1, down1 = cell.forward_flops(out_prev[0][0], alpha, ratio)
                    out.append((out1[1], down1[1] if down1 is not None else None))
                    flops.append([out1[0], down1[0] if down1 is not None else None])
                else:
                    if i == j:
                        out0, down0 = cell.forward_flops(out_prev[j-1][1], alpha, ratio)
                        out.append((out0[1], down0[1] if down0 is not None else None))
                        ...
                        if betas[j][i-j-1][1] > 0:
                            # from keep
                            out1, down1 = cell.forward_flops(out_prev[j][0], alpha, ratio)
                        assert (out0 is None and out1 is None) or out0[1] == out1[1]
                        assert (down0 is None and down1 is None) or down0[1] == down1[1]
                        out.append((out0[1], down0[1] if down0 is not None else None))
                        flops.append([
            ...
            for ii, lat in enumerate(flops):
                ...
                if ii == 0:
                    ...
                    if i == ii:
                        ...
                        if lat[0] is not None: flops_total[ii][0] = betas[j][i-j-1][1] * flops_total[ii][0] + betas[j][i-j-1][0] * flops_total[ii-1][1] + lat[0]
                        if lat[1] is not None: flops_total[ii][1] = betas[j][i-j-1][1] * flops_total[ii][0] + betas[j][i-j-1][0] * flops_total[ii-1][1] + lat[1]
        ###################################
