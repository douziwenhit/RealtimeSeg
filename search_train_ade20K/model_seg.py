...
def mjus2path(mjus, last, layers):
    downs = [0] * layers
    # mjus1 is of length layers-2; beta2: layers-3; beta3: layers-4
    if last == 1:
        down_idx = np.argmax([ beta[0] for beta in mjus[1][1:-1].cpu().numpy() ]) + 1
        downs[down_idx] = 1
    elif last == 2:
        max_prob = 0; max_ij = (0, 1)
        for j in range(layers-4):
            for i in range(1, j-1):
                prob = mjus[1][i][0] * mjus[2][j][0]
                if prob > max_prob:
                    max_ij = (i, j)
                    max_prob = prob
...
def path2widths(path, thetas, width_mult_list):
    widths = []
    for layer in range(1, len(path)):
        scale = path[layer]
        if scale == 0:
            widths.append(width_mult_list[thetas[scale][layer-1].argmax()])
        else:
            widths.append(width_mult_list[thetas[scale][layer-scale].argmax()])
    return widths

def network_metas(alphas, mjus, thetas, width_mult_list, layers, last):
    mjus[1] = F.softmax(mjus[1], dim=-1)
    mjus[2] = F.softmax(mjus[2], dim=-1)
    path = mjus2path(mjus, last, layers)
    widths = path2widths(path, thetas, width_mult_list)
...
class Network_Multi_Path_Infer(nn.Module):
    def __init__(self, alphas, mjus, thetas, num_classes=19, layers=9, criterion=nn.CrossEntropyLoss(ignore_index=-1), Fch=12, width_mult_list=[1.,], stem_head_width=(1., 1.)):
        ...
        self.ops0, self.path0, self.downs0, self.widths0 = network_metas(alphas, mjus, thetas, self._width_mult_list, layers, 0)
        self.ops1, self.path1, self.downs1, self.widths1 = network_metas(alphas, mjus, thetas, self._width_mult_list, layers, 1)
        self.ops2, self.path2, self.downs2, self.widths2 = network_metas(alphas, mjus, thetas, self._width_mult_list, layers, 2)
