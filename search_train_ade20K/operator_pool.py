__all__ = ['ConvNorm', 'Conv3x3', 'DW','DW2','BasicResidual2x',  "DwsBlock", "DWConv",'DW_downup_1x','DW2_downup_2x', "FusedBlock", 'OPS', 'OPS_name', 'OPS_Class']
...
class ConvNorm(nn.Module):
    def __init__(self, C_in, C_out, kernel_size=3, stride=1, padding=None, dilation=1, groups=1, bias=False,
        ...
        if slimmable:
            ...
            self.conv = nn.Conv2d(C_in, C_out, kernel_size, stride, padding=self.padding, dilation=dilation, groups=self.groups, bias=bias)
...
class Conv3x3(nn.Module):
    ...
    def set_ratio(self, ratio):
        assert self.slimmable
        self.ratio = ratio
        self.conv.set_ratio((ratio[0], ratio[1]))
        self.bn.set_ratio(ratio[1])
...
class DW(nn.Module):
    ...
    def set_ratio(self, ratio):
        assert len(ratio) == 2
        self.ratio = ratio
        self.conv1.set_ratio((ratio[0], ratio[0]))
        self.bn1.set_ratio(ratio[0])
        
        self.conv2.set_ratio((ratio[0], ratio[1]))
        self.bn2.set_ratio(ratio[1])

    @staticmethod
    def _flops(h, w, C_in, C_out, kernel_size=3, stride=1, dilation=1, groups=1):
        layer = DW(C_in, C_out, kernel_size, stride, dilation, groups, slimmable=False)
        flops, params = profile(layer, inputs=(torch.randn(1, C_in, h, w),), verbose=False)
        return flops
    ...
    def forward_latency(self, size):
        ...
        if self.stride == 1:
            ...
            h_out = h_in // 2;
            w_out = w_in // 2
        name = "DW_H%d_W%d_Cin%d_Cout%d_stride%d_dilation%d" % (
        h_in, w_in, c_in, c_out, self.stride, self.dilation)
        if name in latency_lookup_table:
            latency = latency_lookup_table[name]
        else:
            print("not found in flops_lookup_table:", name)
            latency = DW._latency(h_in, w_in, c_in, c_out, self.kernel_size, self.stride, self.dilation,
                                               self.groups)
            latency_lookup_table[name] = latency
            np.save(table_file_name, latency_lookup_table)
        return latency, (c_out, h_out, w_out)
    ...
    def forward_flops(self, size):
        ...
        if self.stride == 1:
            ...
            h_out = h_in // 2; w_out = w_in // 2
        name = "DW_H%d_W%d_Cin%d_Cout%d_kernel%d_stride%d" % ( h_in, w_in, c_in, c_out, self.kernel_size, self.stride)
        if name in flops_lookup_table:
            flops = flops_lookup_table[name]
        else:
            print("not found in flops_lookup_table:", name)
            flops = DW._flops(h_in, w_in, c_in, c_out, self.kernel_size, self.stride, self.dilation, self.groups)
            flops_lookup_table[name] = flops
            np.save(table_file_name, flops_lookup_table)
        # if quantize:
        #     flops /= 4
...
class DW_downup_1x(nn.Module):
    def __init__(self, C_in, C_out, kernel_size=3, stride=1, dilation=1, groups=1, slimmable=True,
                 width_mult_list=[1.]):
        super(DW_downup_1x, self).__init__()
        # Both self.conv1 and self.downsample layers downsample the input when stride != 1
        self.C_in = C_in
        self.C_out = C_out
        self.kernel_size = kernel_size
        ...
        self.conv1 = USConv2d(C_in, C_in, 3, 1, padding=dilation, dilation=dilation, groups=C_in, bias=False, width_mult_list=width_mult_list)
        self.bn1 = USBatchNorm2d(C_in , width_mult_list)
    ...
    @staticmethod
    def _flops(h, w, C_in, C_out, kernel_size=3, stride=1, dilation=1, groups=1):
        layer = DW_downup_1x(C_in, C_out, kernel_size, stride, dilation, groups, slimmable=False)
        flops, params = profile(layer, inputs=(torch.randn(1, C_in, h, w),), verbose=False)
        return flops
    ...
    def forward_latency(self, size):
        ...
        if self.stride == 1:
            ...
            h_out = h_in // 2;
            w_out = w_in // 2
        name = "DW_downup_1x_H%d_W%d_Cin%d_Cout%d_stride%d_dilation%d" % (
        h_in, w_in, c_in, c_out, self.stride, self.dilation)
        if name in latency_lookup_table:
            latency = latency_lookup_table[name]
        else:
            print("not found in flops_lookup_table:", name)
            latency = DW_downup_1x._latency(h_in, w_in, c_in, c_out, self.kernel_size, self.stride, self.dilation,
                                               self.groups)
            latency_lookup_table[name] = latency
            np.save(table_file_name, latency_lookup_table)
        return latency, (c_out, h_out, w_out)
    def forward_flops(self, size):
        ...
        if self.stride == 1:
            ...
            h_out = h_in // 2; w_out = w_in // 2
        name = "DW_downup_1x_H%d_W%d_Cin%d_Cout%d_kernel%d_stride%d" % ( h_in, w_in, c_in, c_out, self.kernel_size, self.stride)
        if name in flops_lookup_table:
            flops = flops_lookup_table[name]
        else:
            print("not found in flops_lookup_table:", name)
            flops = DW_downup_1x._flops(h_in, w_in, c_in, c_out, self.kernel_size, self.stride, self.dilation, self.groups)
            flops_lookup_table[name] = flops
            np.save(table_file_name, flops_lookup_table)
        # if quantize:
        #     flops /= 4
...
class DW2(nn.Module):
    def __init__(self, C_in, C_out, kernel_size=3, stride=1, dilation=1, groups=1, slimmable=True,
        ...
        self.conv2 = USConv2d(C_in , C_in, 1, 1, padding=0, dilation=dilation, groups=groups, bias=False,  width_mult_list=width_mult_list)
        self.bn2 = USBatchNorm2d(C_in, width_mult_list)
            
        self.conv3 = USConv2d(C_in, C_in, 3, 1, padding=dilation, dilation=dilation, groups=C_in, bias=False, width_mult_list=width_mult_list)
        self.bn3 = USBatchNorm2d(C_in , width_mult_list)

        self.conv4 = USConv2d(C_in , C_out, 1, 1, padding=0, dilation=dilation, groups=groups, bias=False,  width_mult_list=width_mult_list)
    ...
    def set_ratio(self, ratio):
        ...
        self.conv1.set_ratio((ratio[0], ratio[0]))
        self.bn1.set_ratio(ratio[0])
        self.conv2.set_ratio((ratio[0], ratio[0]))
        self.bn2.set_ratio(ratio[0]) 
        self.conv3.set_ratio((ratio[0], ratio[0]))
        self.bn3.set_ratio(ratio[0])
        self.conv4.set_ratio((ratio[0], ratio[1]))
        self.bn4.set_ratio(ratio[1])

    @staticmethod
    def _flops(h, w, C_in, C_out, kernel_size=3, stride=1, dilation=1, groups=1):
        layer = DW2(C_in, C_out, kernel_size, stride, dilation, groups, slimmable=False)
        flops, params = profile(layer, inputs=(torch.randn(1, C_in, h, w),), verbose=False)
        return flops
    ...
    def forward_latency(self, size):
        ...
        if self.stride == 1:
            ...
            h_out = h_in // 2;
            w_out = w_in // 2
        name = "DW2_H%d_W%d_Cin%d_Cout%d_stride%d_dilation%d" % (
        h_in, w_in, c_in, c_out, self.stride, self.dilation)
        if name in latency_lookup_table:
            latency = latency_lookup_table[name]
        else:
            print("not found in latency_lookup_table:", name)
            latency = DW2._latency(h_in, w_in, c_in, c_out, self.kernel_size, self.stride, self.dilation,
                                               self.groups)
            latency_lookup_table[name] = latency
            np.save(table_file_name, latency_lookup_table)
        return latency, (c_out, h_out, w_out)
    ...
    def forward_flops(self, size):
        ...
        if self.stride == 1:
            ...
            h_out = h_in // 2;
            w_out = w_in // 2
        name = "DW2_H%d_W%d_Cin%d_Cout%d_kernel%d_stride%d" % (
        h_in, w_in, c_in, c_out, self.kernel_size, self.stride)
        if name in flops_lookup_table:
            flops = flops_lookup_table[name]
        else:
            print("not found in flops_lookup_table:", name)
            flops = DW2._flops(h_in, w_in, c_in, c_out, self.kernel_size, self.stride, self.dilation,
                                           self.groups)
            flops_lookup_table[name] = flops
            np.save(table_file_name, flops_lookup_table)
        # if quantize:
...
class DW2_downup_2x(nn.Module):
    def __init__(self, C_in, C_out, kernel_size=3, stride=1, dilation=1, groups=1, slimmable=True,
                 width_mult_list=[1.]):
        super(DW2_downup_2x, self).__init__()
        # Both self.conv1 and self.downsample layers downsample the input when stride != 1
        self.C_in = C_in
        self.C_out = C_out
        self.kernel_size = kernel_size
    ...
    @staticmethod
    def _flops(h, w, C_in, C_out, kernel_size=3, stride=1, dilation=1, groups=1):
        layer = DW2_downup_2x(C_in, C_out, kernel_size, stride, dilation, groups, slimmable=False)
        flops, params = profile(layer, inputs=(torch.randn(1, C_in, h, w),), verbose=False)
        return flops
    ...
    def forward_latency(self, size):
        ...
        if self.stride == 1:
            ...
            h_out = h_in // 2;
            w_out = w_in // 2
        name = "DW2_downup_2x_H%d_W%d_Cin%d_Cout%d_stride%d_dilation%d" % (
        h_in, w_in, c_in, c_out, self.stride, self.dilation)
        if name in latency_lookup_table:
            latency = latency_lookup_table[name]
        else:
            print("not found in latency_lookup_table:", name)
            latency = DW2_downup_2x._latency(h_in, w_in, c_in, c_out, self.kernel_size, self.stride, self.dilation,
                                               self.groups)
            latency_lookup_table[name] = latency
            np.save(table_file_name, latency_lookup_table)
        return latency, (c_out, h_out, w_out)
    ...
    def forward_flops(self, size):
        ...
        if self.stride == 1:
            ...
            h_out = h_in // 2;
            w_out = w_in // 2
        name = "DW2_downup_2x_H%d_W%d_Cin%d_Cout%d_kernel%d_stride%d" % (
        h_in, w_in, c_in, c_out, self.kernel_size, self.stride)
        if name in flops_lookup_table:
            flops = flops_lookup_table[name]
        else:
            print("not found in flops_lookup_table:", name)
            flops = DW2_downup_2x._flops(h_in, w_in, c_in, c_out, self.kernel_size, self.stride, self.dilation,
                                           self.groups)
            flops_lookup_table[name] = flops
            np.save(table_file_name, flops_lookup_table)
        # if quantize:
...
class DwsBlock(nn.Module):
    def __init__(self, C_in, C_out, kernel_size=3, stride=1, dilation=1, groups=1, slimmable=True,
        ...
        if self.slimmable:
            ...
            self.conv2 = nn.Conv2d(C_in*4, C_in*4, 3, stride, padding=dilation, dilation=dilation, groups=C_in,  bias=False)
...
class FusedBlock(nn.Module):
    def __init__(self, C_in, C_out, kernel_size=3, stride=1, dilation=1, groups=1, slimmable=True,
        ...
        if self.slimmable:
            ...
            self.conv1 = nn.Conv2d(C_in, C_in*4, 3, stride, padding=dilation, dilation=dilation, groups=groups, bias=False)
            self.bn1 = BatchNorm2d(C_in*4)
            ...
            self.conv2 = nn.Conv2d(C_in*4, C_out, 1, 1, padding=0, dilation=dilation, groups=groups, bias=False)
...
OPS = {
   # 'skip': lambda C_in, C_out, stride, slimmable, width_mult_list: FactorizedReduce(C_in, C_out, stride, slimmable, width_mult_list),
    'mbconv': lambda C_in, C_out, stride, slimmable, width_mult_list: MBCBlock(C_in, C_out, kernel_size=3, stride=stride, dilation=1, slimmable=slimmable, width_mult_list=width_mult_list),
    'dw': lambda C_in, C_out, stride, slimmable, width_mult_list: DW(C_in, C_out, kernel_size=3,stride=stride, dilation=1,slimmable=slimmable, width_mult_list=width_mult_list),
    'dw2': lambda C_in, C_out, stride, slimmable, width_mult_list: DW2(C_in, C_out, kernel_size=3,stride=stride, dilation=1,slimmable=slimmable, width_mult_list=width_mult_list),
    'dw_dowmup_2x': lambda C_in, C_out, stride, slimmable, width_mult_list: DW2_downup_2x(C_in, C_out, kernel_size=3,stride=stride, dilation=1,slimmable=slimmable, width_mult_list=width_mult_list),
    'dw_dowmup_1x': lambda C_in, C_out, stride, slimmable, width_mult_list: DW_downup_1x(C_in, C_out, kernel_size=3,  stride=stride, dilation=1,   slimmable=slimmable,  width_mult_list=width_mult_list),
     }
OPS_name = ["MBCBlock","DW2_downup_2x","DW", "DW2","DW_downup_1x"]
OPS_Class = OrderedDict()

#OPS_Class['skip'] = FactorizedReduce
OPS_Class['mbconv'] = MBCBlock
OPS_Class['dw'] = DW
OPS_Class['dw2'] = DW2
OPS_Class['dw_dowmup_2x'] = DW2_downup_2x
OPS_Class['dw_dowmup_1x'] = DW_downup_1x
